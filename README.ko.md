...
- ❌ "무료 LLM API 목록"을 찾았지만 2년 전 자료라 링크 절반이 죽어있음
- ❌ 제공업체에 가입했는데 신용카드나 전화 인증을 요구함
- ❌ 드디어 키를 받았지만 설정 도구(Claude Code, Cursor, Codex)마다 다른 형식을 요구
- ❌ 두 무료 모델을 비교하려고 브라우저 탭 8개를 열어 각각 문서를 확인

**이 저장소 + [freellm.net](https://freellm.net)이 모든 문제를 해결합니다.**
